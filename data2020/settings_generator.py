...
for state in state_list:
    out = open(os.path.join(ini_path, f'{state}.ini'),'w')
    outfile =   ''
    outfile +=  '[Inputs]\n'
    outfile += f'geometry: data2020/spatial_network/{scope}/{state.lower()}_tl_2020_{scope}.json\n'
    ...
    outfile +=  f'data: data2020/partisan_data/{scope}/{state.lower()}_{scope}_census_2020_voter_data_2016.csv GEOID D16 D1\n'
    outfile +=  f'data: data2020/partisan_data/{scope}/{state.lower()}_{scope}_census_2020_voter_data_2020.csv GEOID D20 D2\n'
    outfile +=  f'data: data2020/partisan_data/{scope}/{state.lower()}_{scope}_census_2020_voter_data_2016.csv GEOID R16 R1\n'
    outfile +=  f'data: data2020/partisan_data/{scope}/{state.lower()}_{scope}_census_2020_voter_data_2020.csv GEOID R20 R2\n'
    ...
    outfile +=  'population_tolerance: 0.01\n'
    outfile +=  '#accept: \n'
    ...
    outfile += f'summary_file: outputs/summaries/{state}_{scope}_2020_summary_new_objectives.csv\n'
    out.write(outfile)
